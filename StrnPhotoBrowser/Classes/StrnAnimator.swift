...
@objc public protocol StrnPhotoBrowserAnimatorDelegate {
    func willPresent(_ browser: StrnPhotoBrowser)
    func willDismiss(_ browser: StrnPhotoBrowser)
}

class StrnAnimator: NSObject, StrnPhotoBrowserAnimatorDelegate {
    ...
    var animationDuration: TimeInterval {
        if StrnPhotoBrowserOptions.bounceAnimation {
            return 0.5
        }
        return 0.35
    }
...
private extension StrnAnimator {
    ...
    func calcFinalFrame(_ imageRatio: CGFloat) -> CGRect {
        if StrnMesurement.screenRatio < imageRatio {
            let width = StrnMesurement.screenWidth
            let height = width / imageRatio
            let yOffset = (StrnMesurement.screenHeight - height) / 2
            return CGRect(x: 0, y: yOffset, width: width, height: height)
        } else {
            let height = StrnMesurement.screenHeight
            let width = height * imageRatio
            let xOffset = (StrnMesurement.screenWidth - width) / 2
...
private extension StrnAnimator {
    func presentAnimation(_ browser: StrnPhotoBrowser, completion: ((Void) -> Void)? = nil) {
        browser.view.isHidden = true
    ...
    func dismissAnimation(_ browser: StrnPhotoBrowser, completion: ((Void) -> Void)? = nil) {
        UIView.animate(
            withDuration: animationDuration,
            delay:0,
            usingSpringWithDamping:animationDamping,
