...
open class StrnZoomingScrollView: UIScrollView {
    var captionView: StrnCaptionView!
    var photo: StrnPhotoProtocol! {
        didSet {
    ...
    fileprivate(set) var photoImageView: StrnDetectingImageView!
    fileprivate weak var photoBrowser: StrnPhotoBrowser?
    fileprivate var tapView: StrnDetectingView!
    fileprivate var indicatorView: StrnIndicatorView!
    ...
    func setup() {
        // tap
        tapView = StrnDetectingView(frame: bounds)
        ...
        // image
        photoImageView = StrnDetectingImageView(frame: frame)
        ...
        // indicator
        indicatorView = StrnIndicatorView(frame: frame)
...
//        }
    ...
    open func displayImage(complete flag: Bool) {
        ...
        if let image = photo.underlyingImage {
            ...
            // create padding
            // let width: CGFloat = image.size.width + StrnPhotoBrowserOptions.imagePaddingX
            // let height: CGFloat = image.size.height + StrnPhotoBrowserOptions.imagePaddingY;
            ...
            photoImageView.backgroundColor = StrnPhotoBrowserOptions.backgroundColor
...
extension StrnZoomingScrollView: UIScrollViewDelegate {
...
// MARK: - StrnDetectingImageViewDelegate

extension StrnZoomingScrollView: StrnDetectingViewDelegate {
    func handleSingleTap(_ view: UIView, touch: UITouch) {
        guard let browser = photoBrowser else {
            return
        }
        guard StrnPhotoBrowserOptions.enableZoomBlackArea == true else {
            return
        }
        
        if browser.areControlsHidden() == false && StrnPhotoBrowserOptions.enableSingleTapDismiss == true {
    ...
    func handleDoubleTap(_ view: UIView, touch: UITouch) {
        if StrnPhotoBrowserOptions.enableZoomBlackArea == true {
...
extension StrnZoomingScrollView: StrnDetectingImageViewDelegate {
    func handleImageViewSingleTap(_ touchPoint: CGPoint) {
        guard let browser = photoBrowser else {
            return
        }
        if StrnPhotoBrowserOptions.enableSingleTapDismiss {
            browser.determineAndClose()
        } else {
            browser.toggleControls()
        }
...
private extension StrnZoomingScrollView {
